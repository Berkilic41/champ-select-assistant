...
fn combo_count_matches_json() {
    ...
    // 2026-06-14 (Faz 1): +421 audited mekanik-doğru sinerji → 544 (123 baz + 421).
    assert_eq!(dir.len(), 544, "combos.json should have 544 entries");
}
...
fn lane_phase_advice_aram_returns_none() {
    use crate::draft_iq::narrative::build_lane_phase_advice;
...
#[test]
fn lane_phase_advice_tailors_to_opponent_archetype() {
    use crate::draft_iq::narrative::build_lane_phase_advice;
    // Same candidate (late scaler) vs two early bullies of DIFFERENT classes → both
    // freeze, but the opponent-specific caution differs (no fabricated mechanics).
    let scaler = b5_make_archetype("control_mage", "low", vec![], 0.40, 0.85);
    let assassin = b5_make_archetype("assassin", "high", vec![], 0.75, 0.55);
    let juggernaut = b5_make_archetype("juggernaut", "low", vec![], 0.75, 0.55);

    let vs_assassin = build_lane_phase_advice(&scaler, Some(&assassin), "middle").expect("advice");
    let vs_juggernaut =
        build_lane_phase_advice(&scaler, Some(&juggernaut), "middle").expect("advice");

    assert!(vs_assassin.contains("freeze") && vs_juggernaut.contains("freeze"));
    assert_ne!(
        vs_assassin, vs_juggernaut,
        "advice must be tailored to the opponent archetype"
    );
    assert!(vs_assassin.contains("roam"), "got: {vs_assassin}");
    assert!(vs_juggernaut.contains("kite"), "got: {vs_juggernaut}");
}
